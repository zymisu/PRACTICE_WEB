...
function findQuartile(sortedArr, x) {
  ...
  if (idx === -1) return `${x} không tồn tại trong mảng ở câu 3`;
  const partSize = Math.floor(sortedArr.length / 4); // = 5
  const q = Math.min(Math.floor(idx / partSize) + 1, 4);
  const lo = (q - 1) * partSize, hi = q * partSize - 1;
  return `x = ${x} ở index ${idx} → thuộc Q${q} (index ${lo}–${hi})`;
}
...
function runLab03() {
  ...
  addSection('Câu 2: Tổng các phần tử ở câu 1', 'Tổng: ' + sumArray(original));

  addSection('Câu 3: Sắp xếp (Bubble Sort)',
    'Tăng dần : ' + sorted.join(', '));

  // Câu 4 & 5 dùng chung 1 giá trị x duy nhất
  const x = sorted[Math.floor(sorted.length / 2)]; 

  addSection('Câu 4: Tìm kiếm nhị phân',
    (() => {
      const i = binSearch(sorted, x);
      return i !== -1
        ? `x = ${x}\nBinSearch(${x}) → tìm thấy tại index ${i} trong mảng đã sắp xếp câu 3`
        : `x = ${x}\nBinSearch(${x}) → không tìm thấy`;
    })());

  addSection('Câu 5: Tứ phân vị (Quartile)',
    findQuartile(sorted, x));
